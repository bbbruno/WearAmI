  ...
  peisk_nInetInterfaces=0;
    ...
    for (ifa = ifAddrStruct; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if(ifa->ifa_addr->sa_family!=AF_INET) {
            ...
            if(peisk_printLevel & PEISK_PRINT_CONNECTIONS)
                printf("%s is not an INET interface, skipping\n", ifa->ifa_name);
            continue;
        }
        
        strncpy(peisk_inetInterface[peisk_nInetInterfaces].name,ifa->ifa_name,sizeof(peisk_inetInterface[peisk_nInetInterfaces].name));
        peisk_inetInterface[peisk_nInetInterfaces].ip = *(struct in_addr*)(&((struct sockaddr_in *)ifa->ifa_addr)->sin_addr);
        
        peisk_inetInterface[peisk_nInetInterfaces].netmask = *(struct in_addr*)(&((struct sockaddr_in *)ifa->ifa_netmask)->sin_addr);
        
        peisk_inetInterface[peisk_nInetInterfaces].broadcast = *(struct in_addr*)(&((struct sockaddr_in *)ifa->ifa_dstaddr)->sin_addr);
        ...
        if(ioctl(sockfd, SIOCGIFMTU, &ifr) < 0 ){
            ...
            peisk_inetInterface[peisk_nInetInterfaces].mtu = ifr.ifr_mtu;
        }
        
        if(ioctl(sockfd, SIOCGIFMETRIC, &ifr) < 0){
        }else{
            peisk_inetInterface[peisk_nInetInterfaces].metric = ifr.ifr_metric;
        }
...
int peisk_tcpSendAtomic(PeisConnection *connection,PeisPackage *package,int len) {
  ...
  status=send(connection->connection.tcp.socket,(void*) data,len,MSG_DONTWAIT|0);
...
int peisk_tcpReceiveIncomming(struct PeisConnection *connection,struct PeisPackage *package) {
  ...
  errno=0;
  status=recv(connection->connection.tcp.socket,(void*)package,sizeof(PeisPackage),MSG_PEEK|0);
  ...
  status=recv(connection->connection.tcp.socket,
	      (void*)package,sizeof(package->header) + ntohs(package->header.datalen),0);
