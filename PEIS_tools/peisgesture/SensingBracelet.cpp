...
//! @return:	---
void SensingBracelet::printSetInfo()
{
	for(int i=0; i<nbM; i++)
		setI[i].printInfo();
}
...
//! @param rF:			name of the result file
//! @return:			---
void SensingBracelet::updateIntervalFull(int& i,int& nS,float &p,float &pp,int f,string rF)
{
	// 1) active
	if(p > 0)
	{
...
//! @param &pp:			reference to the model possibility at previous sample
//! @return:			---
void SensingBracelet::updateIntervalSimple(int& i,int& nS,float &p,float &pp)
{
	// 1) active
	if(p > 0.8)
	{
...
//! @return:			---
void SensingBracelet::offlineReasoning(string path, string testFile)
{
	...
	while (!pf.eof())
	{
		...
		for(int i = 0; i < nbM; i++)
			updateIntervalFull(i,nSamples,possibilities[i],past_possibilities[i],0,rF);
