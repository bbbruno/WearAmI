...
void peisk_syncflush(PeisConnection *connection) {
  ...
  case eTCPConnection:
	  while(1) {
	    ...
	    errno=0;
	    status=recv(connection->connection.tcp.socket,(void*)&mysync,sizeof(mysync),MSG_PEEK|0);
	    ...
	    errno=0;
	    //status=recv(connection->connection.tcp.socket,(void*)&mysync,sizeof(mysync),0);
	    status=recv(connection->connection.tcp.socket,(void*)&mysync,1,0);
	  }
	  break;
  case eUDPConnection:
...
int peisk_recvBlocking(int fd,void *buf,size_t len,int flags,double timeout) {
  ...
  while(len>0) {
    errno=0;
    new_bytes=recv(fd,data,len,flags|0);
