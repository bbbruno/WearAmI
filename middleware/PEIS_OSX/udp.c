...
int peisk_udpConnect(char *name,int port,int flags) {
  ...
  /*sendto(sock,(void*)buf,sizeof(int)+strlen(peisk_networkString)+1,0,&connection->connection.udp.addr,connection->connection.udp.len);*/
  ret=sendto(sock,(void*)buf,sizeof(int)+64,0,&connection->connection.udp.addr,connection->connection.udp.len);
...
void peisk_acceptUDPConnections() {
  ...
  sendto(sock,(void*)"ack",4,0,&addr,len);
...
int peisk_udpSendAtomic(PeisConnection *connection,PeisPackage *package,int len) {
  ...
  errno=0;
  /*status=sendto(connection->connection.udp.socket,(void*) data,len,MSG_DONTWAIT|0,&connection->connection.udp.addr,connection->connection.udp.len);*/
  status=sendto(connection->connection.udp.socket,(void*) data,len,0,&connection->connection.udp.addr,connection->connection.udp.len);
  if(status == -1) {
...
int peisk_udpReceiveIncomming(struct PeisConnection *connection,struct PeisPackage *package) {
  errno=0;
  int len=sizeof(connection->connection.udp.addr);
  int size=recvfrom(connection->connection.udp.socket,(void*)package,sizeof(PeisPackage),0,&connection->connection.udp.addr,&len);
  if(size <= 0) return 0;  /* no message received */
  if(memcmp(&connection->connection.udp.addr,&connection->connection.udp.addr,sizeof(struct sockaddr_in)) != 0) {
